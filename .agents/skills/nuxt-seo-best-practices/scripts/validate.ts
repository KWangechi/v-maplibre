...
async function validateSkillFile(): Promise<ValidationError[]> {
  ...
  try {
    ...
    if (!skillContent.includes('name: nuxt-seo-best-practices')) {
      errors.push({
        file: 'SKILL.md',
        error: 'Invalid or missing skill name in frontmatter',
      });
    }
  } catch {
...
async function validate() {
  console.log('Validating nuxt-seo-best-practices...');
